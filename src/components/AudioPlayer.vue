<template>
  <!-- Container -->
  <div
    class="fixed bottom-0 left-0 right-0 w-full max-w-md mx-auto h-52 z-50 bg-neutral-800"
  >
    <!-- Title + Heart -->
    <div class="flex items-center justify-between mx-6 mt-6 mr-6">
      <p class="text-xl font-light text-white">
        {{ episodeStore.currentPlayingEpisode.title }}
      </p>
      <button
        @click="toggleHeart"
        class="text-gray-500 hover:text-orange-500 transition-colors duration-300"
      >
        <svg
          v-if="!isFilled"
