    ...
    <section class="mt-4 bg-white overflow-y-auto">
      <div class="flex items-center justify-between px-4 mb-2">
        ...
        <div class="flex">
          <!-- Flag icons -->
          <img
            @click="changeLanguage('dk')"
            src="/src/assets/flag-dk.svg"
            alt="DK Flag"
            class="h-5 w-8 object-cover mx-1 cursor-pointer"
          />
          <img
            @click="changeLanguage('en')"
            src="/src/assets/flag-gb.svg"
            alt="UK Flag"
            class="h-5 w-8 object-cover mx-1 cursor-pointer"
          />
          <img
            @click="changeLanguage('de')"
            src="/src/assets/flag-gm.svg"
            alt="DE Flag"
            class="h-5 w-8 object-cover mx-1 cursor-pointer"
          />
      ...
      <div v-if="isLoading" class="flex">Loading...</div>
      <div v-if="error" class="flex">Error: {{ error.message }}</div>
      <ul v-if="!isLoading && !error">
        <!-- Iterate over the episodes array to generate list items -->
        <li
          v-for="episode in episodes"
          :key="episode.id"
          class="flex items-center py-3 px-4 border-b border-gray-200"
        >
          <img
            :src="episode.image"
            alt=""
          ...
          />
          <button @click="playSelectedEpisode(episode)">
            <img
          ...
          <div class="ml-4 flex-grow">
            ...
            <span class="font-semibold">{{
              $t(`titles.${idToKeyMap[episode.id]}`)
            }}</span>
          </div>
          <button
            @click="toggleHeart(episode.id)"
            class="text-black hover:text-orange-500 transition-colors duration-300"
          >
            <svg
              v-if="!heartStates[episode.id]"
...
onMounted(() => {
  episodeStore
    .fetchEpisodes()
    .then(() => {
      console.log("Episodes fetched:", episodes.value);
    })
    .catch((err) => {
...
episodes.value.forEach((episode) => {
  ...
  heartStates[episode.id] =
    storedIsFilled !== null ? JSON.parse(storedIsFilled) : false;
});
...
const toggleHeart = (episodeId) => {
  ...
  localStorage.setItem(
    `heartIsFilled_${episodeId}`,
    JSON.stringify(heartStates[episodeId])
  );
};
...
const playSelectedEpisode = (episode) => {
